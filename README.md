...
This repository contains a **customized version for CycleWR**, with an image carousel (snippet-based), category posts section, category posts embed (HTML block), and Get Involved component. See [CycleWR customizations](#cyclewr-customizations) below.
...
- **Category posts component** – A reusable partial that shows the latest 3 posts for a given tag (category). Used on the home page as “Latest from Advocacy” with configurable `tagSlug` and optional `title`. See `partials/components/category-posts.hbs`. To add this block to a page **without using the template dropdown**: use a slug-based template. A page whose URL slug is `with-advocacy` (e.g. `/with-advocacy/`) automatically uses `page-with-advocacy.hbs`, which includes the category-posts block. For another page (e.g. `/about/`), copy that file to `page-about.hbs`. You can also use the **"With category posts"** template from the editor sidebar (Template dropdown) if your Ghost admin shows it. Edit the tag slug and title in the template file (and in `home.hbs`) to change which category is shown. The section title styling was updated for consistency with the rest of the theme.
- **Category posts embed (HTML block)** – Add a grid of posts by tag to any page via the Ghost editor's HTML block. Paste the snippet from `snippets/category-posts-embed.html` and set `data-tag` (tag slug) and `data-api-key` (Content API key from Ghost Admin → Settings → Integrations). Optional: `data-limit` (default 6), `data-title`, `data-no-image="true"`. The theme script `assets/js/category-posts-embed.js` fetches posts from the Content API and renders them using the same `gh-card` structure as the category-posts partial (image above title, full width in content). Excerpts: if the post has a custom excerpt it is shown in full; otherwise the auto excerpt is trimmed to 200 characters with "…". Only the post title is a link to the post. Rebuild the theme (`yarn build`) after changing the snippet or JS so updates appear in `built/source.js` and `built/screen.css`.
- **Get Involved component** – A home page section (`partials/components/get-involved.hbs`) that lists “ways to get involved” from **posts with a specific tag** (e.g. `get-involved`). Each post is shown as a card: post **title** = card title, post **excerpt** = body text, post **URL** = “Learn more” link. Configure the tag in Ghost Admin → Design → Customize → Get Involved (`get_involved_tag`). No per-item custom configs needed.
